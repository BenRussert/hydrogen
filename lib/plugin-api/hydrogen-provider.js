...
import typeof Hydrogen from "../main";
...
export default class HydrogenProvider {
  ...
  constructor(_hydrogen: Hydrogen) {
   ...
   * @return {Disposable} Atom Disposable
   */
  onDidChangeKernel(
    callback: (kernelWrapper: ?HydrogenKernel) => void
  ): IDisposable {
    return this._hydrogen.emitter.on("did-change-kernel", (kernel: ?Kernel) => {
      if (kernel) {
   ...
   * `null` is returned if no active kernel
   * @return {Class} `HydrogenKernel`
   */
  getActiveKernel(): ?HydrogenKernel {
    if (!store.kernel) return null;
